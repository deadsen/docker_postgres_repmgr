...
cp -f /tmp/postgresql.conf /var/lib/postgresql/data/
#echo "port = $PGPORT" >> /var/lib/postgresql/data/postgresql.conf

#createuser -U "$POSTGRES_USER" -s --replication "$REPMGR_USER"
echo "CREATE ROLE $REPMGR_USER WITH LOGIN SUPERUSER REPLICATION PASSWORD '$REPMGR_PASSWORD'" | psql -U "$POSTGRES_USER"
